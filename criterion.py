...
class styleLoss_v2(nn.Module):
    def forward(self, input, target):
        ...
        loss = nn.MSELoss(size_average=True)(mean_x, mean_y) + nn.MSELoss(size_average=True)(var_x, var_y) + nn.MSELoss(
            size_average=True)(iCov, tCov)
        return loss
...
class LapLoss(nn.Module):
    ...
    def forward(self, input, target, reduce='mean'):
       ...
       # return pyr_input[0], pyr_target[0]
        if reduce is 'mean':
