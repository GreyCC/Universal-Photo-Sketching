...
class Encoder(nn.Module):
    ...
    def forward(self, HR):
        ...
        feat_multi_attn, _ = self.cont_attn(feat_multi_attn.permute(2, 0, 1), feat_multi_attn.permute(2, 0, 1),
                                            feat_multi_attn.permute(2, 0, 1))
...
class P2Sv2(nn.Module):
    def __init__(self):
        ...
        self.weighting1 = nn.Parameter(torch.ones(1))
        self.weighting2 = nn.Parameter(torch.ones(1))
    ...
    def forward(self, x):
        ...
        heat_map_pre_norm = self.reassemble(h) * score

        heat_map = heat_map_pre_norm.clone()
        without_norm = x * heat_map_pre_norm

        heat_map[::] -= torch.min(heat_map_pre_norm[::])
        heat_map[::] /= torch.max(heat_map_pre_norm[::])
        with_norm = x * heat_map

        out = (self.weighting1 * with_norm + without_norm * self.weighting2) / 2
