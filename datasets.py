...
class DatasetFromFolderWithoutEdgeGT(data.Dataset):
    def __init__(self, data_dir, ref_dir, fineSize=256):
        super(DatasetFromFolderWithoutEdgeGT, self).__init__()
    ...
    def transform(self, image, mask1):
        # Resize
        resize = transforms.Resize(size=(288, 288))
        image = resize(image)
        mask1 = resize(mask1)
    ...
    def __getitem__(self, index):
        ...
        # input = self.transform(input)
        input, edge = self.transform(input, edge)
        ref = self.style_transform(ref)

        return input, edge, ref

    def __len__(self):
        return self.input_len


class DatasetFromADE20K(data.Dataset):
    def __init__(self, fineSize=512):
        super(DatasetFromADE20K, self).__init__()

        self.data_dir = 'Test/ADE20K'
        self.fineSize = fineSize

        self.input_filenames = sorted(glob(join(self.data_dir, '*.jpg')))
        self.input_len = len(self.input_filenames)
    ...
    def transform(self, image):
        ...
        # Random crop
        i, j, h, w = transforms.RandomCrop.get_params(
            image, output_size=(self.fineSize, self.fineSize))
        image = TF.crop(image, i, j, h, w)
